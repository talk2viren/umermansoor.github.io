...
May be, a UTF-16 `char` type from Oracle is be the answer. Or at least a runtime exception when compiler detects that something's bad about to happen. Until then, we should probably avoid the `char` type. Even its official [JavaDocs](https://docs.oracle.com/javase/7/docs/api/java/lang/Character.html) don't sound all that convincing:
